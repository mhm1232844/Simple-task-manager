...
RSpec.describe "Tasks", type: :request do
  ...
  describe "POST /tasks" do
    # ... (create tests - kept the same) ...
    context "with valid parameters" do
      let(:valid_attributes) { attributes_for(:task, title: "Valid Task Title") }
      it "creates a new Task" do
        expect { post tasks_path, params: { task: valid_attributes } }.to change(Task, :count).by(1)
      end
    ...
    context "with invalid parameters" do
      ...
      it "does not create a new Task" do
        expect { post tasks_path, params: { task: invalid_attributes } }.to change(Task, :count).by(0)
      end
  ...
  describe "PATCH /tasks/:id/toggle_complete" do
    # ... (toggle tests - kept the same, including redirect on not found) ...
    let(:task) { create(:task, completed: false) }
    it "updates the task's completed status from false to true" do
      patch toggle_complete_task_path(task); task.reload
      expect(task.completed).to be true
    end
     it "updates the task's completed status from true to false" do
      task.update!(completed: true); patch toggle_complete_task_path(task); task.reload
    ...
    it "handles non-existent tasks gracefully by redirecting" do
      patch toggle_complete_task_path(id: task.id + 999)
      expect(response).to redirect_to(tasks_path)
  ...
  describe "GET /tasks/:id/edit" do
    # ... (edit tests - kept the same, including redirect on not found) ...
  ...
  describe "PATCH /tasks/:id" do
    # ... (update tests - kept the same, including redirect on not found) ...
    ...
    context "with valid parameters" do
      ...
      it "updates the requested task" do
        patch task_path(task), params: { task: new_attributes }; task.reload
    ...
    context "with invalid parameters" do
      ...
      it "does not update the task" do
         original_title = task.title; patch task_path(task), params: { task: invalid_attributes }; task.reload
  ...
  describe "DELETE /tasks/:id" do
    let!(:task) { create(:task, title: "Task to Delete") } # Use let!
    ...
    it "returns a :see_other status code on redirect" do
      delete task_path(task)
      expect(response).to have_http_status(:see_other)
    end

    # Corrected expectation for non-existent task on delete
    it "handles non-existent tasks gracefully by redirecting" do
      delete task_path(id: task.id + 999) # Non-existent ID
      expect(response).to redirect_to(tasks_path)
      expect(flash[:alert]).to eq("Task not found.")
    end
